...
try {
    ...
    $sortField = isset($requestData['sortBy']) ? $requestData['sortBy'] : 'nama_zona';
    $sortOrder = isset($requestData['sortOrder']) ? $requestData['sortOrder'] : 'ASC';

    // Validate and sanitize sortField to prevent SQL injection
    $allowedSortFields = ['nama_zona'];
    if (!in_array($sortField, $allowedSortFields)) {
        $sortField = 'nama_zona'; // Default to waktu_mulai_kegiatan if the provided field is not allowed
    }

    // Construct the SQL query with filtering conditions
    $query = "SELECT * FROM zona WHERE is_deleted = 0";
