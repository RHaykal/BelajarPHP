...
try {
    ...
    if ($filter) {
        if (isset($filter->nama_pangkat)) {
            $filterConditions[] = "nama_pangkat LIKE :nama_pangkat";
        }
    ...
    if ($filter) {
        if (isset($filter->nama_pangkat)) {
            $nama_pangkat = '%' . $filter->nama_pangkat . '%';
            $stmt->bindParam(':nama_pangkat', $nama_pangkat, PDO::PARAM_STR);
        }
    ...
    $totalCountQuery = "SELECT COUNT(*) FROM pangkat";
