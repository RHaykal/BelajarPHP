...
try {
    ...
    $filterNipJaksaPenuntut = isset($requestData['filter']['nip']) ? $requestData['filter']['nip'] : "";
    $filterNamaJaksaPenuntut = isset($requestData['filter']['nama_jaksa']) ? $requestData['filter']['nama_jaksa'] : "";


    // Determine the sorting parameters (assuming they are passed as query parameters)
    $sortField = isset($requestData['sortBy']) ? $requestData['sortBy'] : 'nama_jaksa';
    $sortOrder = isset($requestData['sortOrder']) ? $requestData['sortOrder'] : 'ASC';

    // Validate and sanitize sortField to prevent SQL injection
    $allowedSortFields = ['nama_jaksa', 'nip'];
    if (!in_array($sortField, $allowedSortFields)) {
        $sortField = 'nama_jaksa'; // Default to waktu_mulai_kegiatan if the provided field is not allowed
    }

    // Construct the SQL query with filtering conditions
    $query = "SELECT * FROM jaksa_penuntut WHERE is_deleted = 0";
    ...
    if (!empty($filterNipJaksaPenuntut)) {
        $query .= " AND jaksa_penuntut.nip LIKE '%$filterNipJaksaPenuntut%'";
    }

    if (!empty($filterNamaJaksaPenuntut)) {
        $query .= " AND jaksa_penuntut.nama_jaksa LIKE '%$filterNamaJaksaPenuntut%'";
    }



    $query .= " GROUP BY jaksa_penuntut.jaksa_penuntut_id";
    ...
    foreach ($res as $row) {
        $recordData = [
            'jaksa_penuntut_id' => $row['jaksa_penuntut_id'],
            'nip' => $row['nip'],
