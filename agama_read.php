...
try {
    ...
    if ($filter) {
        if (isset($filter->nama_agama)) {
            $filterConditions[] = "nama_agama LIKE :nama_agama";
        }
    ...
    if ($filter) {
        if (isset($filter->nama_agama)) {
            $nama_agama = '%' . $filter->nama_agama . '%';
            $stmt->bindParam(':nama_agama', $nama_agama, PDO::PARAM_STR);
        }
    ...
    $totalCountQuery = "SELECT COUNT(*) FROM agama";
