...
try {
    ...
    $sortField = isset($requestData['sortBy']) ? $requestData['sortBy'] : 'nama_penugasan';
    $sortOrder = isset($requestData['sortOrder']) ? $requestData['sortOrder'] : 'ASC';

    // Validate and sanitize sortField to prevent SQL injection
    $allowedSortFields = ['nama_penugasan'];
    if (!in_array($sortField, $allowedSortFields)) {
        $sortField = 'nama_penugasan'; // Default to waktu_mulai_kegiatan if the provided field is not allowed
    }

    // Construct the SQL query with filtering conditions
    $query = "SELECT * FROM penugasan WHERE is_deleted = 0";
    ...
    $query .= " GROUP BY penugasan.penugasan_id";
