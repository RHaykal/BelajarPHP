...
try {
    ...
    $filterKasusId = isset($requestData['filter']['kasus_id']) ? $requestData['filter']['kasus_id'] : "";


    // Determine the sorting parameters (assuming they are passed as query parameters)
    $sortField = isset($requestData['sortBy']) ? $requestData['sortBy'] : 'nama_saksi';
    $sortOrder = isset($requestData['sortOrder']) ? $requestData['sortOrder'] : 'ASC';

    // Validate and sanitize sortField to prevent SQL injection
    $allowedSortFields = ['nama_saksi', 'kasus_id'];
    if (!in_array($sortField, $allowedSortFields)) {
        $sortField = 'nama_saksi'; // Default to waktu_mulai_kegiatan if the provided field is not allowed
    }

    // Construct the SQL query with filtering conditions
    $query = "SELECT * FROM saksi WHERE is_deleted = 0";
    ...
    if (!empty($filterKasusId)) {
        $query .= " AND saksi.kasus_id LIKE '%$filterKasusId%'";
    }


    $query .= " GROUP BY saksi.saksi_id";
