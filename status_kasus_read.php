...
try {
    ...
    // Memeriksa token di tabel user_login
    ...
    $filterNamaStatusKasus = isset($requestData['filter']['nama_status_kasus']) ? $requestData['filter']['nama_status_kasus'] : "";


    // Determine the sorting parameters (assuming they are passed as query parameters)
    $sortField = isset($requestData['sortBy']) ? $requestData['sortBy'] : 'nama_status_kasus';
    $sortOrder = isset($requestData['sortOrder']) ? $requestData['sortOrder'] : 'ASC';

    // Validate and sanitize sortField to prevent SQL injection
    $allowedSortFields = ['nama_status_kasus'];
    if (!in_array($sortField, $allowedSortFields)) {
        $sortField = 'nama_status_kasus'; // Default to waktu_mulai_kegiatan if the provided field is not allowed
    }

    // Construct the SQL query with filtering conditions
    $query = "SELECT * FROM status_kasus WHERE is_deleted = 0";
    ...
    if (!empty($filterNamaStatusKasus)) {
        $query .= " AND nama_status_kasus LIKE '%$filterNamaStatusKasus%'";
    }


    $query .= " GROUP BY status_kasus_id";
    ...
    foreach ($res as $row) {
        $recordData = [
            ...
            'nama_status_kasus' => $row['nama_status_kasus']
        ];
        $record[] = $recordData;
    }
