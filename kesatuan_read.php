...
try {
    ...
    $sortField = isset($requestData['sortBy']) ? $requestData['sortBy'] : 'nama_kesatuan';
    $sortOrder = isset($requestData['sortOrder']) ? $requestData['sortOrder'] : 'ASC';

    // Validate and sanitize sortField to prevent SQL injection
    $allowedSortFields = ['nama_kesatuan', 'nama_lokasi_kesatuan'];
    if (!in_array($sortField, $allowedSortFields)) {
        $sortField = 'nama_kesatuan'; // Default to waktu_mulai_kegiatan if the provided field is not allowed
    }

    // Construct the SQL query with filtering conditions
    $query = "SELECT kesatuan.*, 
    lokasi_kesatuan.nama_lokasi_kesatuan
    FROM kesatuan
    JOIN lokasi_kesatuan ON kesatuan.lokasi_kesatuan_id = lokasi_kesatuan.lokasi_kesatuan_id
    ...
    $query .= " GROUP BY kesatuan.kesatuan_id";
