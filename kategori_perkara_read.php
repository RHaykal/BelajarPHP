...
try {
    ...
    if ($filter) {
        if (isset($filter->nama_kategori_perkara)) {
            // $filterConditions[] = "nama_kategori_perkara LIKE :nama_kategori_perkara";
            $filternamakategori = '%' . $filter->nama_kategori_perkara . '%';
        }
    ...
    if (!empty($filternamakategori)) {
        $whereClause .= " AND nama_kategori_perkara LIKE :nama_kategori_perkara";
    }

    // Modify your SQL query to include pagination and filtering
    $query = "SELECT * FROM kategori_perkara";
    ...
    if ($filter) {
        if (isset($filter->nama_kategori_perkara)) {
            $nama_kategori_perkara = '%' . $filter->nama_kategori_perkara . '%';
            $stmt->bindParam(':nama_kategori_perkara', $nama_kategori_perkara, PDO::PARAM_STR);
        }
    ...
    $totalCountQuery = "SELECT COUNT(*) FROM kategori_perkara";
