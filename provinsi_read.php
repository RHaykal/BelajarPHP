...
try {
    ...
    $pageSize = isset($request_data->pageSize) ? (int) $request_data->pageSize : 10000; // Number of records per page
    ...
    if ($filter) {
        if (isset($filter->nama_provinsi)) {
            $filterConditions[] = "nama_provinsi LIKE :nama_provinsi";
        }
    ...
    if ($filter) {
        if (isset($filter->nama_provinsi)) {
            $nama_provinsi = '%' . $filter->nama_provinsi . '%';
            $stmt->bindParam(':nama_provinsi', $nama_provinsi, PDO::PARAM_STR);
        }
    ...
    $totalCountQuery = "SELECT COUNT(*) FROM provinsi";
