...
try {
    ...
    $filterNipHakim = isset($requestData['filter']['nip']) ? $requestData['filter']['nip'] : "";
    $filterNamaHakim = isset($requestData['filter']['nama_hakim']) ? $requestData['filter']['nama_hakim'] : "";
    $filterDepartemen = isset($requestData['filter']['departemen']) ? $requestData['filter']['departemen'] : "";


    // Determine the sorting parameters (assuming they are passed as query parameters)
    $sortField = isset($requestData['sortBy']) ? $requestData['sortBy'] : 'nama_hakim';
    $sortOrder = isset($requestData['sortOrder']) ? $requestData['sortOrder'] : 'ASC';

    // Validate and sanitize sortField to prevent SQL injection
    $allowedSortFields = ['nama_hakim', 'nip', 'departemen'];
    if (!in_array($sortField, $allowedSortFields)) {
        $sortField = 'nama_hakim'; // Default to waktu_mulai_kegiatan if the provided field is not allowed
    }

    // Construct the SQL query with filtering conditions
    $query = "SELECT * FROM hakim WHERE is_deleted = 0";
    ...
    if (!empty($filterNipHakim)) {
        $query .= " AND hakim.nip LIKE '%$filterNipHakim%'";
    }

    if (!empty($filterNamaHakim)) {
        $query .= " AND hakim.nama_hakim LIKE '%$filterNamaHakim%'";
    }

    if (!empty($filterDepartemen)) {
        $query .= " AND hakim.departemen LIKE '%$filterDepartemen%'";
    }


    $query .= " GROUP BY hakim.hakim_id";
